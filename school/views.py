...
def change_teacher_password(request, teacher_id):
    ...
    if request.method == 'POST':
        ...
        if new_password and len(new_password) >= 6:
            ...
            teacher.user.save()
            messages.success(request, f'Password changed for {teacher.user.first_name}!')
            return redirect('headmaster_teachers')
        messages.error(request, 'Password must be at least 6 characters.')
    return render(request, 'headmaster/change_password.html', {'teacher': teacher})
...
def academic_history(request, student_id):
    student = get_object_or_404(Student, id=student_id)
    history = AcademicHistory.objects.filter(student=student).order_by('-created_at')
...
def post_marks(request, class_id):
    ...
    if request.method == 'POST':
        ...
        if subject_id and term_id:
            ...
            messages.success(request, 'Marks posted successfully!')
            return redirect('teacher_dashboard')
        messages.error(request, 'Please select subject and term!')
    
    return render(request, 'teacher/post_marks.html', {
        'class_obj': class_obj,
        'students': students,
        'subjects': subjects,
        'terms': terms,
...
def update_school_profile(request):
    ...
    if request.method == 'POST':
        ...
        profile.opening_date = request.POST.get('opening_date')
        profile.closing_date = request.POST.get('closing_date')
